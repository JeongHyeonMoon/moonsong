...
function readReviewWithPaging() {
	sendGet('/api/member/myreviews?start-index=' + startReviewIndex + '&offset='
			+ offset, {}, function(success) {
				
		if (success.length > 0) {
			startReviewIndex += min(offset, success.length);
		}
...
function renderHtml(htmlString, data) {
	...
	if (memberId != undefined && memberId == data.writerId) {
		htmlElement.children('#review-controller-' + data.reviewId).append(
