...
<c:if test="${requestScope.AuthInformation.authorized == true}">
	<a href='javascript:openNewPopUp("/reviews/add/");'
...
<div id='review-data-format' style='display: none;'>
	<div id='review-data-%review_id%' class='list-group-item media'>
		...
		<div class='media-right'>
			<div>
				<c:if test="${requestScope.AuthInformation.authorized }">
					<button class='btn btn-default'
						onclick='javascript:sendReviewReport("%review_id%");'>
						신고하기</button>
				</c:if>
			</div>
			<div id='review-controller-%review_id%'></div>
		</div>
	</div>
</div>
...
<ul id='tag-format' class='breadcrumb' style='display: none;'>
	<li><a href='javascript:searchKeyword("%tag_name%");'>#%tag_name%</a></li>
</ul>